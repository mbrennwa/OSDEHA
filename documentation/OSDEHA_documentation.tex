...
The CCS load is implemented as a ``gyrator'' circuit\cite{mogliaa_gyrator}, which uses an adjustable voltage reference to set the DC bias voltage at the DHT anodes and a cascoded depletion-mode FET that works as a high-impedance CCS in the AC domain. The constant current output from each CCS is shared between the DHT and the headphone stator. Therefore, the current flowing through the DHT will exhibit a slight AC variation in response to the current draw by the stator. However, this AC current variation will be small (less than \SI{4}{mA})\cite{osdeha_p8}, and the general concept for the operating point illustrated in \figr{801A_curves} remains valid. The CCS can also be configured to operate as a $\mu$-follower\cite{pimm_ccs,kimmel_mustage}, where the stator current is taken directly from the source pin of the CCS FET. This configuration restores the constant-current operation of the DHT tube with a perfectly flat load line exactly as shown in \figr{801A_curves}.
