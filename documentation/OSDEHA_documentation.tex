...
\figr{system_overview} shows an overview the OSDEHA. There various units are distributed across two chassis (power supply, amplifier).

Power-supply chassis:
\begin{itemize}
\item IEC mains inlet (with power switch and mains fuses)
\item Two mains transformers
\item Main power-supply board that supplies stabilized DC voltages for B+, B-, B2-, diaphragm bias, and heater of the input-stage tubes
\item Two boards for raw DC supply to the Coleman DHT filament regulators
\end{itemize}
...
\item Four Colemand DHT filament regulators
\item Two XLR audio input sockets
\item Volume control (stereo, balanced)
\item Stax headphone output socket
\end{itemize}

The two chassis are connected by two umbilical cable U1 and U2. U1 carries GND, chassis/earth, and the high-voltage/low-current supplies. U2 carries the low-voltage/high-current supplies for the heaters of the input-stage tubes and the Coleman DHT filament regulators.

\begin{figure}
\begin{center}
\includegraphics[angle=90,height=0.9\textheight]{system_boards_overview.pdf}
\caption{Overview of the OSDEHA units (see text).}
...
\subsubsection{High-voltage supplies (B+, B-, B2-)}
TO DO

\subsubsection{Diaphragm bias voltage}
TO DO

\subsubsection{Heater supply (input-stage tubes)}
TO DO

\subsubsection{DHT filament supply}
TO DO -- RAW DC / COLEMAN DHT FILAMENT REGS (IN AMP CHASSIS)
