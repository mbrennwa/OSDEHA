...
\begin{itemize}
  ...
  % \item Input sensitivity for full output: \SI{XYZ}{\unitVpp}
  \item Input impedance (balanced): $2 \times \SI{10}{k\Omega}$
  % \item Voltage gain: $XYZ \times$
  \item Frequency response with \SI{100}{pF} load ($-\SI{0.5}{dB}$)\\
        $\mu$-output: \SI{28}{Hz} to \SI{33}{kHz}\\
        Anode-output: \SI{24}{Hz} to \SI{21}{kHz}\\
        See \figr{frequency_response}
  ...
  \item Harmonic distortion: $< \SI{0.04}{\%}$ at \SI{100}{\unitVrms} / \SI{282}{\unitVpp} output\\
        See \figr{harmonics_vs_voltage} 
...
\begin{figure}
\begin{center}
\includegraphics[width=0.8\textwidth]{frequency_response.pdf}
\caption{Frequency response, measured with a \SI{100}{pF} load connected to the output.}
...
\begin{figure}
\begin{center}
\includegraphics[width=0.8\textwidth]{harmonics_vs_voltage.pdf}
\caption{Harmonic distortion vs.\ output voltage of a \SI{1}{kHz} sine test signal, measured with a \SI{100}{pF} load connected to the $\mu$ output (top) or the anode output (bottom).}
