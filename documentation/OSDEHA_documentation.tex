...
\begin{itemize}
  \item Max output: \SI{1640}{V peak-to-peak} into \SI{100}{pF}
  
\end{itemize}

% . Power Output
%
%     RMS Power Output (Watts per Channel @ Ohms) – Continuous power an amplifier can deliver per channel (e.g., 100W RMS per channel @ 8Ω).
%     Peak Power Output – Maximum short-term power handling capability.
%     Total Power Output – Total RMS power across all channels.
%
% 2. Load Impedance
%
%     Specifies the supported speaker impedances (e.g., 4Ω, 8Ω).
%     Some amplifiers support bridged mode operation with different impedances.
%
% 3. Frequency Response
%
%     The range of frequencies the amplifier can reproduce accurately, typically given in Hz – kHz (e.g., 20Hz – 20kHz ±0.5dB).
%
% 4. Total Harmonic Distortion (THD)
%
%     Measures how much the amplifier distorts the signal, usually expressed as a percentage (e.g., <0.1% @ 1kHz, full power).
%     Lower THD means cleaner sound.
%
% 5. Signal-to-Noise Ratio (SNR)
%
%     Ratio of signal power to noise power, usually in dB (e.g., >100dB).
%     Higher values indicate less background noise.
%
% 6. Damping Factor
%
%     The ability of the amplifier to control speaker movement, typically given as a ratio (e.g., >200 @ 8Ω).
%     Higher values indicate better speaker control.
%
% 7. Input Sensitivity & Input Impedance
%
%     Input Sensitivity: Voltage needed to drive the amp to full power (e.g., 1.5V RMS).
%     Input Impedance: Resistance to the input signal, usually in kΩ (e.g., 47kΩ).
%
% 8. Crosstalk
%
%     Measures signal leakage between channels, expressed in dB (e.g., <-90dB @ 1kHz).
%     Lower values indicate better channel separation.
%
% 9. Intermodulation Distortion (IMD)
%
%     Indicates how well the amp handles multiple frequencies simultaneously (e.g., <0.05%).
%     Lower values mean better fidelity.
%
% 10. Output Impedance
%
%     Determines compatibility with different speaker loads (usually low, e.g., <0.1Ω).
%
% 11. Gain
%
%     Amplification factor, given in dB (e.g., 26dB).
%
% 12. Efficiency (for Class D & Class AB)
%
%     Expressed as a percentage (e.g., 90% for Class D, 60% for Class AB).
%
% 13. Power Supply & Consumption
%
%     Voltage Requirements (e.g., 120V AC, 50/60Hz).
...
%     Thermal shutdown.
%
% 15. Physical & Environmental Specs
%
%     Dimensions (W × H × D, mm or inches).
%     Weight (kg or lbs).
