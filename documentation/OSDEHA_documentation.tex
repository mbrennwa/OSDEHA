...
\subsection{Active and passive electronic parts}
The details of the tubes, semiconductors, resistors and capacitors used in the OSDEHA circuits are given in the KiCad files in the {\tt kicad/} directory.

\subsection{Chassis and Enclosures}
The chassis and enclosures are built from custom-made panels manufactured using the FrontPanel Express service.\cite{SchaefferAG} using the FrontDesign files in the {\tt chassis/} directory. The additional parts used to construct the chassis and mount the OSDEHA units are listed in the table below.

TABLE / LIST HERE (autogenerated from chassis partslist CSV)

\csvautobooktabular{../chassis/chassis_partslist.csv}
