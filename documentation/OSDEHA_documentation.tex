...
\begin{itemize}
\item Determine the CCS sense-resistor values that yield the targeted bias currents for the CCSs with the specific DN2540 parts used in the input LTP tail and in the DHT grid drivers. The setup shown in \figr{CCS_Rsense_jig} may be useful.
...
\begin{figure}
\begin{center}
\includegraphics[height=0.2\textheight]{CCS_Rsense_circuit}
\caption{Circuit to determine the sense-resistor value $R_{\textrm s}$ that yields the desired CCS current for a given DN2540 part ($Q_{\textrm x}$). Adjust $R_{\textrm s}$ until the voltage drop across $R_{\textrm s}$ corresponds to the desired current. The test voltage $V_{+}$ should not be less than about \SI{5}{V}.}
