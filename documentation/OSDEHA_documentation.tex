...
\caption{Circuit to determine the sense-resistor value $R_{\textrm s}$ that yields the desired CCS current for a given DN2540 part ($Q_{\textrm x}$). Adjust $R_{\textrm s}$ until the voltage $V_{\textrm x}$ corresponds to the desired current. The test voltage $V_{+}$ should not be less than about \SI{5}{V}.}
